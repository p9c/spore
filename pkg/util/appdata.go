...
func GetDataDir(goos, appName string, roaming bool) string {
	...
	// OSes if the directory from the Go standard lib failed.
	if err != nil || homeDir == "" {
	...
	case "windows":
		// Windows XP and before didn't have a LOCALAPPDATA, so fallback to regular
		// APPDATA when LOCALAPPDATA is not set.
...
//
// An empty appName or one with a single dot is treated as requesting the
// current directory so only "." will be returned. Further, the first character
